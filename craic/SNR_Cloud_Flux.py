...
def sigmoid_blend(E, pflux_low, pflux_high, E1=0.05, E2=0.15, a=0.5):
    ...
    pflux_low : array_like
        Spectrum from low-energy regime (~ GeV^-1 cm^-3)
    pflux_high : array_like
        Spectrum from high-energy regime (~ GeV^-1 cm^-3)
    E1 : float
    ...
    Ec = 0.5 * (E1 + E2)
    delta = (E2 - E1) / a  # Smaller a = smoother transition
...
class SNR_Cloud_Flux:
    ...
    def _compute_low_energy_gamma(self, phi, pfluxlow_tmp) -> 1 / (u.TeV*u.s*u.cm**2):
        ...
        pflux_total = sigmoid_blend(self.Egs.value, phig_norm * phig_low.value, phi.value) / (u.TeV * u.cm**2 * u.s) # TeV
