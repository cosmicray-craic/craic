...
class SNR_Cloud_Flux:
    ...
    def _compute_travel_parameters(self, nh2, dist, age) -> tuple[u.cm, u.pc, u.yr, u.pc]:
        ...
        """
        # Compute Escape time
        tesc = acce.escape_time(Ep=Eps, typeIIb=self.snr_typeII)
        ...
        if self.accel_type == 'Impulsive':
            if np.isclose(sum(dism).value, 0):
                pflux_tmp = injection.compute_pflux_impulsive_extended(
                    Resc, self.N0, Eps, dism, time_diff, nh2, chi=self.chi, ism=1, alpha=part.alpha)
                pfluxlow_tmp = injection.compute_pflux_impulsive_extended(
                    Resc, self.N0, Epls, dism, time_diff, nh2, chi=self.chi, ism=1, alpha=part.alpha)
            else:
                pflux_tmp = injection.compute_pflux_impulsive_extended(
                    Resc, self.N0, Eps, dism, time_diff, acce.nh2ism, chi=self.chi, ism=1, alpha=part.alpha)
                pfluxlow_tmp = injection.compute_pflux_impulsive_extended(
                    Resc, self.N0, Epls, dism, time_diff, acce.nh2ism, chi=self.chi, ism=1, alpha=part.alpha)
        
        elif self.accel_type == 'Continuous':
            pflux_tmp = injection.compute_pflux_continuous_extended(
                Resc, self.N0, Eps, dism, ismtime, acce.nh2ism, chi=self.chi, ism=1, alpha=part.alpha)
            pfluxlow_tmp = injection.compute_pflux_continuous_extended(
                Resc, self.N0, Epls, dism, ismtime, acce.nh2ism, chi=self.chi, ism=1, alpha=part.alpha)
    ...
    def _add_galactic_flux(self, pflux_tmp) -> u.GeV**-1*u.cm**-3:
        ...
        if self.F_gal is not False:
            fgal_AMSO2 = injection.compute_fgal(Eps)
            fgal_DAMPE = injection.compute_fgal_dampe(Eps)
